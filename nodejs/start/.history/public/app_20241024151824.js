...
document.addEventListener('click', (event) => {
    ...
    if (event.target.dataset.type === "edit") {
        ...
        // Находим текстовый узел, используя свойство childNodes
        let noteTextElement = Array.from(listItem.childNodes).find(node => node.nodeType === Node.TEXT_NODE);

        const id = event.target.dataset.id;  // Используем dataset для получения id

        // Если текстовой узел найден, продолжаем
        if (noteTextElement) {
            ...
            console.error("Не удалось найти текстовый узел в элементе <li>");
        }
    }
});
