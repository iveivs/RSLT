...
function App() {
    ...
    const [isDeleting, setIsUpdating] = useState(false);
    ...
    const requestUpdateSomeProduct = () => {
        setIsUpdating(true);
        fetch("http://localhost:3000/products/2", {
            ...
            .then((rawResponse) => rawResponse.json())
            .then((response) => {
                console.log("Товар обновлён ответ сервера" , response);
                refreshProducts();
            })
    ...
    return (
        <>
            ...
            <button disabled={isUpdating} onClick={requestUpdateSomeProduct}>
                Обновить определённый товар
            </button>
            <button disabled={isDeleting} onClick={requestDeleteSomeProduct}>
                Удалить определённый товар
            </button>
        </>
    );
}
