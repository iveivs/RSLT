...
function App() {
	...
	const { isUpdating, requestUpdateTodo } = useRequestUpdateTodo(refreshProducts, currentTodo)
	...
	return (
		<>
			...
			{isLoading ? (
				...
				todos.map((todo) => (
					<div  key={todo.id}>
						<p>task: {todo.task} id: {todo.id} {todo.complited === 'true' ? '✅' : '❌'}</p>
						<button>Удалить</button>
						<button onClick={() => {requestUpdateTodo, setCurrentTodo(todo)}}>Поменять статус задачи</button>
					</div>
				))
			)}
		</>
