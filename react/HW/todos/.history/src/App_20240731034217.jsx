...
function App() {
	...
	const { isUpdating, requestUpdateTodo } = useRequestUpdateTodo(refreshProducts, id, task, complited)
			...
			{isLoading ? (
				...
				todos.map(({ id, task, complited }) => (
					<div  key={id}>
						...
						<button onClick={requestUpdateTodo(id, task, complited)}>Поменять статус задачи</button>
					</div>
				))
			)}
		</>
