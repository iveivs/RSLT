...
function App() {
	...
	const { requestUpdateTodo } = useRequestUpdateTodo(refreshProducts, currentTodo)
			...
			{isLoading ? (
				...
				todos.map((todo) => (
					<div  key={todo.id}>
						...
						<button onClick={() => {setCurrentTodo(todo), requestUpdateTodo(todo)}}>Поменять статус задачи</button>
					</div>
				))
			)}
		</>
