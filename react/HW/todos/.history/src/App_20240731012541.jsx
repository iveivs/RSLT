...
function App() {
	...
	const { isCreating, requestAddSomeTodo } = useRequestAddSomeTodo(refreshProducts, input);

	return (
		<>
			<h1>Todos</h1>
			{/* < TodosList /> первое задание */}
			< Form input={input} setInput={setInput} requestAddSomeTodo={requestAddSomeTodo}/>
			{isLoading ? (
				<div className="loader"></div>
			) : (
				todos.map(({ id, task }) => <div key={id}>task: {task} id:{id}</div>)
			)}
		</>
	);
}
