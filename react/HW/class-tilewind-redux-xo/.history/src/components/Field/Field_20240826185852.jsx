...
import FieldLayout from './FieldLayout'; // Импортируем как экспорт по умолчанию
...
class Field extends Component {
    ...
    handleClick(i) {
        ...
        if (checkDraw(updatedField)) {
            SET_IS_DRAW(); // Вызов метода dispatch
        }
        if (isWin) {
            SET_GAME_END(); // Вызов метода dispatch
        } else {
            SET_CURRENT_PLAYER(); // Вызов метода dispatch
        }
    }
...
const mapDispatchToProps = (dispatch) => ({
    ...
    SET_CURRENT_PLAYER: () => dispatch(SET_CURRENT_PLAYER), // Вызов функции для получения экшена
    SET_GAME_END: () => dispatch(SET_GAME_END), // Вызов функции для получения экшена
    SET_IS_DRAW: () => dispatch(SET_IS_DRAW()), // Вызов функции для получения экшена
});
