...
// Добавьте в класс метод под названием repairCar() , который будет чинить машину. 
// Он должен принимать в себя 1 параметр carName - имя машины для ремонта (тип данных string).

// Если carName не был передан, то отобразите ошибку: “Вам необходимо указать название машины, 
// чтобы ее отремонтировать”, в консоли через console.error(). На этом выполнение функции должно заканчиваться.

// Если же параметр carName был передан в repairCar(), то вам необходимо сравнить текущее часы с временем работы автосервиса.
//  Если текущее время (часы) не входит в диапазон работы автосервиса, то отобразите через alert() сообщение: “К сожалению, 
//  мы сейчас закрыты. Приходите завтра”.
//   Иначе же выводите сообщение: “Сейчас отремонтируем вашу машину carName ! Ожидайте пожалуйста” (carName - параметр метода repairCar()).

// Примечание: при сравнении времени берите в учет только часы, на минуты не обращайте внимание.
//  Пусть минуты всегда будут строкой “00”. Для теста работоспособности класса CarService используйте данный код:
// const carService = new CarService('RepairCarNow', { from: '8:00', till: '20:00' });
// carService.repairCar('BMW');

// Подсказка: для того чтобы извлечь из строки “8:00” значение часов, вы можете использовать split(':'), 
// получить первый элемент из полученного массива и преобразовать его к числу.
