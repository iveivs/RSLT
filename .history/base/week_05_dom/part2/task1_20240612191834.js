...
modalWindow.addEventListener('click', e =>{
    ...
    if(target.classList.contains('delete-modal__confirm-button')){
        tasks = tasks.filter(item => item.id !== curTaskId)
...
document.addEventListener('keydown', (e) => {
    ...
    if(!themeFlag){
        ...
        document.querySelectorAll('.task-item').forEach(item => item.style.color = '#fff')
        document.querySelectorAll('button').forEach(item => item.style.border = '1px solid #ffffff')
    } else {
        document.body.style.background = 'initial'
        document.querySelectorAll('.task-item').forEach(item => item.style.color = 'initial')
        document.querySelectorAll('button').forEach(item => item.style.border = 'none')
    }    
})
