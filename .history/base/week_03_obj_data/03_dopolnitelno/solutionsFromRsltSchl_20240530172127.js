...
// Вам необходимо случайным образом выбрать победный номерок (случайный ключ в объекте applicants) и вернуть из 
// функции getWinner() объект, в котором будут хранится свойства из winnerObject и объект победителя.
...
Ваш заказчик - визовый центр. У них есть база данных клиентов, но нет скрипта, который бы автоматизировал процесс выдачи виз. 
Поэтому визовый центр попросил вас помочь как веб-разработчика взять эту задачу на себя.

Вам необходимо создать функцию allowVisa(), которая будет фильтровать переданный массив из клиентов визового центра.
 Она принимает в себя массив клиентов. Каждый клиент - это объект с параметрами firstName (имя клиента), lastName (фамилия клиента), criminalRecord (тип данных boolean, есть ли судимость), passportExpiration (тип данных string, дата истечения срока действия паспорта).
