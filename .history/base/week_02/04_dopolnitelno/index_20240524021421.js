...
function getMathResult(expression) {
    ...
    let newExpression
    // if(!arrOfOperators.includes(expression[1])) {
    //     return `Ошибка`
    // }
    if(expression.length > 3) {
        newExpression = expression.filter(item => {
            ...
            if(item === arrOfOperators.find(elem => elem === itemm)){
                console.log(item);
                return item
            }
        })
    }
    // const firstNum = Number(expression[0])
    // const secondNum = Number(expression[2])
