...
let numberOfGoals = Math.max.apply(null, goals)

let number = goals.indexOf(numberOfGoals) + 1
console.log(`Самый результативный матч был под номером ${number}. В нем было забито ${numberOfGoals} гол(ов).`);
...
function getMinimalGoals(arr) {
    ...
    console.log(sameMinimalMatch);
    return sameMinimalMatch.splice(',')
}

console.log(getMinimalGoals(goals));