...
const coffees = ['Latte', 'searchedCoffee', 'Americano'];
let coffeeName = prompt(`Поиск кофе по названию:`)
const searchedCoffee = coffees.map((elem, index) => {
    if(coffeeName.toLowerCase() === elem.toLowerCase()){
        return [elem, index + 1]
    }
})
console.log(searchedCoffee);
alert(`Держите ваш любимый кофе ${searchedCoffee[0][0]}. Он ${searchedCoffee[0][1]}-й по популярности в нашей кофейне.`)